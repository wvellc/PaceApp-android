...
fun GenderTypes.getDefaultGaits(): Pair<GaitPace, GaitPace> {
    return when (this) {
        GenderTypes.MALE -> Pair(
            GaitPace(value = DEFAULT_MALE_WALK, unit = GaitUnit.FEET),
            GaitPace(value = DEFAULT_MALE_RUN, unit = GaitUnit.FEET)
        )
        GenderTypes.FEMALE -> Pair(
            GaitPace(value = DEFAULT_FEMALE_WALK, unit = GaitUnit.FEET),
            GaitPace(value = DEFAULT_FEMALE_RUN, unit = GaitUnit.FEET)
        )
        else -> Pair(
            GaitPace(value = DEFAULT_OTHER_WALK, unit = GaitUnit.FEET),
            GaitPace(value = DEFAULT_OTHER_RUN, unit = GaitUnit.FEET)
        )
    }
}